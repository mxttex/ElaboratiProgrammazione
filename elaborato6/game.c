...
void move_ball(void) {
	...
	struct position ball_dir = get_ball_dir();
	struct position newBallPos;

	if (ball_dir.x == RIGHT)
		newBallPos.x = ball_pos.x + 1;
	else
		newBallPos.x = ball_pos.x - 1;

	if (ball_dir.y == DOWN)
		newBallPos.y = ball_pos.y + 1;
	else
		newBallPos.y = ball_pos.y - 1;
	game.ball_pos = newBallPos;
	game.ball_dir = check_collision(game.ball_pos);
}
