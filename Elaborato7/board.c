...
int check_map_corners(unsigned int coord) {
...
int num_neighbor_mines(int board[][GAME_COLS], unsigned int ro, unsigned int col) {
    ...
    int i, j;
    for (i = check_map_corners(ro-1); i <= check_map_corners(ro+1); i++) {
        for (j = check_map_corners(col-1); j <= check_map_corners(col+1); j++) {
...
int display_cell(int board[][GAME_COLS], unsigned int ro, unsigned int col, int num_disp_cells) {
    ...
    switch (mines) {
        ...
        case 0:
            ...
            for (i = check_map_corners(ro - 1); i <= check_map_corners(ro + 1); i++) {
                for (j = check_map_corners(col - 1); j <= check_map_corners(col + 1); j++) {
                    next_num_disp_cells += (board[i][j] == UNKN_FREE);
...
int expand_board(int board[][GAME_COLS], unsigned int rows, unsigned int cols, unsigned int i, unsigned int j) {
    ...
    if (board[i][j] != UNKN_FREE && board[i][j] != UNKN_MINE){
        for (k = check_map_corners(i - 1); k <= check_map_corners(i + 1); k++){
            for (f = check_map_corners(j - 1); f <= check_map_corners(j + 1); f++){
                ...
                if(board[k][f] == UNKN_MINE && neighbor_flags == board[i][j])
                    return -1;
            }
        }
